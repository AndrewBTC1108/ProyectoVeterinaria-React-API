...
import DoctorUser from '../../components/Doctor/DoctorUser'
import useSWR from 'swr';
import Spinner from '../../components/Spinner';
export default function DoctorUsers() {
    const [page, setPage] = useState(1);
    const [searchTerm, setSearchTerm] = useState('');
    const {handleClickModalDeworming, handleClickModalVaccine, handlesetIdUser} = useAmorPorTi()
                    ...
                    <tbody>
                        {usersData.data.map(userdata => (
                            <DoctorUser
                                ...
                                modalVac={{handleClickModalVaccine}}
                                modalDew={{handleClickModalDeworming}}
                            />
                        ))}
                    </tbody>
                )}
            ...
            {usersData && (
                <div className="my-1 flex justify-center space-x-2">
                    <button
                        ...
                        disabled={page === 1}//when page == 1 disabled button
